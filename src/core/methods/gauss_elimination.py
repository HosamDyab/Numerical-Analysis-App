...
class GaussEliminationMethod(NumericalMethodBase):
    def solve(self, matrix_str: str, vector_str: str, decimal_places: int = 6) -> Tuple[List[float], List[Dict]]:
        ...
        Following the book's approach exactly.
        """
        try:
            ...
            for i in range(n):
                ...
                if abs(A[i, i]) < 1e-10:
                    return None, [{"Error": "Zero pivot encountered. Try rearranging the equations."}]
            ...
            return None, [{"Error": f"Error in Gauss Elimination: {str(e)}"}]
